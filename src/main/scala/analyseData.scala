...
object analyseData extends App {
  override def main(args: Array[String]) {
    ...
    val inputFolders = Array(inputFolderName + folderSeparator + "en" + folderSeparator)
    ...
    //resQuery.show(false)
    val savedTable = resQuery.coalesce(2)
    //resQuery.orderBy(desc("")).show(false)

    savedTable.write.parquet(outputFolderName + folderSeparator + "en")
